c.StartList("halo_announcer_cameo")
L["mommy milkers"]={{path="chatsounds/autoadd/halo_announcer_cameo/mommy milkers.ogg",length=2.127}}
L["no maidens"]={{path="chatsounds/autoadd/halo_announcer_cameo/no maidens.ogg",length=2.813}}
L["redditor"]={{path="chatsounds/autoadd/halo_announcer_cameo/redditor.ogg",length=1.309}}
L["skill issue"]={{path="chatsounds/autoadd/halo_announcer_cameo/skill issue.ogg",length=2.449}}
L["social credit deducted"]={{path="chatsounds/autoadd/halo_announcer_cameo/social credit deducted.ogg",length=3.445}}
L["that was sus"]={{path="chatsounds/autoadd/halo_announcer_cameo/that was sus.ogg",length=2.575}}
c.EndList()